...
const OrderTracking = () => {
    ...
    const executeTrack = async (ordNum, ph) => {
        ...
        try {
            ...
            setError(err.response?.data?.error || 'تعذر العثور على الطلب. يرجى التأكد من الرقم والمحاولة مرة أخرى.');
            setOrder(null);
        } finally {
            setLoading(false);
        }
    ...
    const statusMap = {
        'pending': { label: 'في الانتظار', icon: Clock, color: 'text-amber-500 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/20' },
        'confirmed': { label: 'تم التأكيد', icon: CheckCircle2, color: 'text-blue-500 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-900/20' },
        'processing': { label: 'قيد التجهيز', icon: Package, color: 'text-purple-500 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-900/20' },
        'shipped': { label: 'تم الشحن', icon: Truck, color: 'text-indigo-500 dark:text-indigo-400', bg: 'bg-indigo-50 dark:bg-indigo-900/20' },
        'delivered': { label: 'تم التوصيل', icon: CheckCircle2, color: 'text-green-500 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-900/20' },
        'cancelled': { label: 'ملغي', icon: AlertCircle, color: 'text-red-500 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-900/20' },
        'returned': { label: 'مرتجع', icon: AlertCircle, color: 'text-gray-500 dark:text-gray-400', bg: 'bg-gray-50 dark:bg-gray-800' },
    };
                    ...
                    <form onSubmit={handleTrack} className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex items-end">
                            <button
                                ...
                                className="w-full md:w-auto bg-gold-500 hover:bg-gold-600 text-white px-10 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-gold-500/20 disabled:bg-gray-300 dark:disabled:bg-dark-600"
                            >
                                {loading ? 'جاري البحث...' : (
                                    <>
                                        <Search size={20} />
                                ...
                                <div className="relative">
                                    ...
                                    <div className="space-y-12">
                                        ...
                                        <div className="relative pr-12">
                                            <div className="absolute right-0 top-0 w-8 h-8 rounded-full bg-cream-100 dark:bg-dark-700 border-2 border-gold-200 dark:border-dark-600 flex items-center justify-center z-10 text-gold-400">
                                                <CheckCircle2 size={16} />
                                            </div>
                                            <div>
                                                <div className="flex justify-between items-center mb-1">
                                                    <h4 className="font-bold text-text-secondary dark:text-gold-400">تم استلام الطلب</h4>
                                                    <span className="text-xs text-text-muted dark:text-gold-400/60 font-poppins">{new Date(order.created_at).toLocaleString('ar-LY')}</span>
                                                </div>
