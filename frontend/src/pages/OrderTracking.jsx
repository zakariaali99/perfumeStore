        ...
        <div className="bg-cream-50 min-h-screen pt-24 pb-20">
            <div className="container mx-auto px-4 max-w-4xl">
                <div className="text-center mb-12">
                    ...
                    <p className="text-text-secondary">أدخل رقم الطلب لمتابعة حالة شحنتك لحظة بلحظة</p>
                </div>
                ...
                <div className="bg-white p-8 rounded-3xl border border-gold-100 shadow-sm mb-10">
                    <form onSubmit={handleTrack} className="flex flex-col md:flex-row gap-4">
                        ...
                        <div className="flex-1 space-y-2">
                            <label className="text-xs font-bold text-text-secondary px-1 uppercase tracking-wider">رقم الهاتف (اختياري)</label>
                            <input
                                type="tel"
                                value={phone}
                    ...
                    {order && (
                        ...
                        >
                            ...
                            <div className="bg-white p-8 rounded-3xl border border-gold-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                                ...
                                <div className="h-12 w-px bg-gold-100 hidden md:block"></div>
                                <div className="text-center">
                                    <p className="text-sm text-text-secondary mb-1">رقم الطلب</p>
                                    <p className="text-xl font-black font-poppins">{order.order_number}</p>
                                </div>
                                <div className="h-12 w-px bg-gold-100 hidden md:block"></div>
                                <div className="text-center md:text-left">
                                    <p className="text-sm text-text-secondary mb-1">تاريخ الطلب</p>
                                    <p className="font-bold">{new Date(order.created_at).toLocaleDateString('ar-LY')}</p>
                                </div>
                            </div>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="space-y-12">
                                                ...
                                                <div key={i} className="relative pr-12">
                                                    ...
                                                    <div>
                                                        <div className="flex justify-between items-center mb-1">
                                                            <h4 className={`font-bold ${i === 0 ? 'text-text-primary' : 'text-text-secondary'}`}>{s?.label}</h4>
                                                            <span className="text-xs text-text-muted">{new Date(h.created_at).toLocaleString('ar-LY')}</span>
                                                        </div>
                                                        <p className="text-sm text-text-secondary">{h.notes}</p>
                                                    </div>
                                                </div>
                                            );
                                        })}
                                        ...
                                        <div className="relative pr-12">
                                            ...
                                            <div>
                                                <div className="flex justify-between items-center mb-1">
                                                    <h4 className="font-bold text-text-secondary">تم استلام الطلب</h4>
                                                    <span className="text-xs text-text-muted">{new Date(order.created_at).toLocaleString('ar-LY')}</span>
                                                </div>
                                                <p className="text-sm text-text-secondary">شكراً لثقتكم بنا، طلبكم قيد المعالجة.</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    {order.items?.map((item, i) => (
                                        <div key={i} className="py-4 flex justify-between items-center">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-cream-50 rounded-xl overflow-hidden">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-text-secondary">{item.variant_size} مل × {item.quantity}</p>
                                                </div>
                                            </div>
                                            <span className="font-bold font-poppins">{item.total_price} د.ل</span>
                                        </div>
                                    ))}
                                    <div className="pt-6 flex justify-between items-center text-lg font-black">
                                        <span>إجمالي المدفوع</span>
                                        <span className="text-gold-700 font-poppins">{order.total} د.ل</span>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
