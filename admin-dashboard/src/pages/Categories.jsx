...
const Categories = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (currentCategory) {
                await adminApi.updateCategory(currentCategory.id, data);
        ...
        <div className="p-8 space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">إضافة وتعديل تصنيفات المنتجات في المتجر</p>
                </div>
                <button
                ...
                <table className="w-full text-right">
                    <thead className="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th className="px-6 py-4 text-sm font-bold text-gray-900">الرابط (Slug)</th>
                            <th className="px-6 py-4 text-sm font-bold text-gray-900">الترتيب</th>
                            <th className="px-6 py-4 text-sm font-bold text-gray-900">الحالة</th>
                            <th className="px-6 py-4 text-sm font-bold text-gray-900">الإجراءات</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        {categories.map((cat) => (
                            <tr key={cat.id} className="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4 font-bold text-gray-900">{cat.name_ar}</td>
                                <td className="px-6 py-4 text-sm text-gray-500 font-poppins">{cat.slug}</td>
                                <td className="px-6 py-4 text-sm text-gray-500">{cat.order}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${cat.is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                            <h2 className="text-xl font-bold text-gray-900">{currentCategory ? 'تعديل فئة' : 'إضافة فئة جديدة'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-gold-500 outline-none"
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border-none rounded-xl py-3 px-4 focus:ring-2 focus:ring-gold-500 outline-none font-poppins"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-200 border-dashed rounded-2xl relative">
                                    {imagePreview ? (
